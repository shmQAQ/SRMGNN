...
class MoleculeDataset(object):
    def __init__(self, dataset, degree=True, k_hop=2, se="khopgnn", use_subgraph_edge_attr=True,
    ...
    def compute_degree(self):
        ...
        for g in self.dataset:
            deg = 1. / \
                torch.sqrt(1. + utils.degree(g.edge_index[0], g.num_nodes))
    ...
    def extract_subgraphs(self):
        ...
        for i in range(len(self.dataset)):
            ...
            for node_idx in range(graph.num_nodes):
                ...
                indicators.append(torch.zeros(
                    sub_nodes.shape[0]).fill_(node_idx))
                if self.use_subgraph_edge_attr and graph.edge_attr is not None:
                    # CHECK THIS DIDN"T BREAK ANYTHING
    ...
    def __getitem__(self, index):
        ...
        if self.return_complete_index:
            data.complete_edge_index = torch.vstack(
                (s.repeat_interleave(n), s.repeat(n)))
        data.degree = None
        if self.degree:
        ...
        if self.se == "khopgnn":
            if self.cache_path is not None:
                cache_file = torch.load(
                    "{}_{}.pt".format(self.cache_path, index))
                data.subgraph_edge_index = cache_file['subgraph_edge_index']
                data.num_subgraph_nodes = len(
                    cache_file['subgraph_node_index'])
            ...
            data.subgraph_indicator = self.subgraph_indicator_index[index].type(
                torch.LongTensor)
        else:
